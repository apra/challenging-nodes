...
class vaemodel(nn.Module):
    ...
    def traversal(self, x=None, samples=1, out_dir: Path = None):
        ...
        fps = 20
        b_size = samples
        means = torch.zeros((b_size, self.latent_size)).to("cuda")
        sigma = torch.ones((b_size, self.latent_size)).to("cuda")
        logvars = torch.zeros((b_size, self.latent_size)).to("cuda")
        ...
        for i, base_code in enumerate(means):
            ...
            visualize_util.save_animation(
                np.transpose(np.array(images), (0, 1, 3, 4, 2)), filename, fps
            )
        ...
        for i, base_code in enumerate(means):
            ...
            for j in range(base_code.shape[0]):
                ...
                code[:, j] = visualize_util.cycle_gaussian(
                    ...
                    num_frames,
                    loc=loc,
                    scale=10 * np.sqrt(total_variance),
                )
        ...
        for i, base_code in enumerate(means):
            ...
            for j in range(base_code.shape[0]):
                ...
                code[:, j] = visualize_util.cycle_interval(
                    base_code[j], num_frames, -2.0, 2.0
                )
                output = self.decoder(torch.Tensor(code).to("cuda")).cpu().numpy()
                output = (output - np.min(output)) / (np.max(output) - np.min(output))
                output *= 255
            ...
            filename = out_dir / Path(
                "fixed_interval_cycle/fixed_interval_cycle%d.gif" % i
            )
            filename.parent.mkdir(parents=True, exist_ok=True)
            visualize_util.save_animation(
                np.transpose(np.array(images), (0, 1, 3, 4, 2)), filename, fps
        ...
        for i, base_code in enumerate(means):
            ...
            for j in range(base_code.shape[0]):
                ...
                code[:, j] = visualize_util.cycle_interval(
                    base_code[j], num_frames, loc - 2.0 * scale, loc + 2.0 * scale
                )
                output = self.decoder(torch.Tensor(code).to("cuda")).cpu().numpy()
                output = (output - np.min(output)) / (np.max(output) - np.min(output))
                output *= 255
                ...
                images.append(output)
            filename = out_dir / Path(
                "conf_interval_cycle/conf_interval_cycle%d.gif" % i
            )
            filename.parent.mkdir(parents=True, exist_ok=True)
            visualize_util.save_animation(
                np.transpose(np.array(images), (0, 1, 3, 4, 2)), filename, fps
        ...
        for i, base_code in enumerate(means):
            ...
            for j in range(base_code.shape[0]):
                ...
                images.append(output)
            filename = out_dir / Path(
                "minmax_interval_cycle/minmax_interval_cycle%d.gif" % i
            )
            filename.parent.mkdir(parents=True, exist_ok=True)
            visualize_util.save_animation(
                np.transpose(np.array(images), (0, 1, 3, 4, 2)), filename, fps
