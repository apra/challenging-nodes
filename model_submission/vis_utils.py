...
def grid_save_images(images, image_path):
    ...
    image_rows = [
        np.concatenate(images[side_length * i : side_length * i + side_length], axis=0)
...
def padded_grid(images, num_rows=None, padding_px=10, value=None):
    ...
    rows = [
        padded_stack(
            all_images[i * num_cols : (i + 1) * num_cols], padding_px, 1, value=value
        )
...
def cycle_factor(starting_index, num_indices, num_frames):
    ...
    grid = np.linspace(
        starting_index, starting_index + 2 * num_indices, num=num_frames, endpoint=False
    )
...
def cycle_gaussian(starting_value, num_frames, loc=0.0, scale=1.0):
    """Cycles through the quantiles of a Gaussian in a single cycle."""
    starting_prob = stats.norm.cdf(starting_value, loc=loc, scale=scale)
    grid = np.linspace(
        starting_prob, starting_prob + 2.0, num=num_frames, endpoint=False
    )
    grid -= np.maximum(0, 2 * grid - 2)
    grid += np.maximum(0, -2 * grid)
    grid = np.minimum(grid, 0.999)
    grid = np.maximum(grid, 0.001)
    return np.array([stats.norm.ppf(i, loc=loc, scale=scale) for i in grid])
...
def cycle_interval(starting_value, num_frames, min_val, max_val):
    ...
    grid = np.linspace(
        starting_in_01, starting_in_01 + 2.0, num=num_frames, endpoint=False
    )
    grid -= np.maximum(0, 2 * grid - 2)
    grid += np.maximum(0, -2 * grid)
