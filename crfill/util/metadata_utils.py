...
def metadata_dict_node21(metadata_location):
    ...
    with open(metadata_location) as f_obj:
        reader = csv.reader(f_obj, delimiter=",")
        for line in reader:
            _, h, img_name, _, w, x, y = [
                int(entry) if entry.isnumeric() else entry for entry in line
            ]
            if h != 0 and w != 0:  # only append nonempty bboxes
                img_name = str(Path(img_name))  # compatibility between different OS
                bboxes.setdefault(
                    img_name, []
                )  # these two lines allow safe placing of multiple values for key
            ...
            if is_image_file(fname):
                ...
                if chex_or_mimic:
                    image_dir = os.path.join(
                        Path(image_dir), Path("")
                    )  # adds slashes at end of name appropriate to OS
                    image_dir = str(image_dir)[:-1]  # remove '.' at end
                    fname = path.replace(
                        image_dir, ""
                    )  # adds to the filename the necessary directories to find the image in the dict
...
def get_paths_and_nodules(
    ...
    total_image_nodule_list += get_paths_and_nodules_helper(
        node21_image_dir, chex_or_mimic=False
    )
    ...
    if include_chexpert:
        total_image_nodule_list += get_paths_and_nodules_helper(
            chex_image_dir, chex_or_mimic=True
        )
    if include_mimic:
        total_image_nodule_list += get_paths_and_nodules_helper(
            mimic_image_dir, chex_or_mimic=True
        )
