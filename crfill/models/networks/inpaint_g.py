...
class TwostagendGenerator(BaseNetwork):
    ...
    def modify_commandline_options(parser, is_train):
        parser.add_argument("--baseG", type=str, default="baseconv", help="baseg")
        opt, unknown = parser.parse_known_args()
        network = find_class_in_module(
            opt.baseG + "generator", "models.networks.inpaint_g"
        )
    ...
    def __init__(self, opt):
        ...
        self.sconv2 = gen_conv(
            2 * cnum, 4 * cnum, 3, 1, activation=nn.ReLU()
        )  # skip cnn out
...
class DeepFillGenerator(BaseNetwork):
    ...
    def modify_commandline_options(parser, is_train):
        parser.add_argument(
            "--use_cam", action="store_true", help="use context attention module"
        )
        parser.add_argument(
            "--no_mk", action="store_true", help="use context attention module"
        )
    ...
    def __init__(self, opt, return_feat=False):
        ...
        self.cam_2 = ReduceContextAttentionP2(
            ufstride=2 * rate, bkg_patch_size=4 * rate, stride=2 * rate, pd=0, mk=mk
        )
    ...
    def get_param_list(self, stage="all"):
        if stage == "all":
            ...
            list_param = [
                p for name, p in self.named_parameters() if (name.startswith("conv"))
            ]
            return list_param
        elif stage == "fine":
            list_param = [
    ...
    def forward(self, x, mask):
        ...
        x = x * mask + xin[:, 0 : self.img_channel, :, :] * (1.0 - mask)
        xnow = x
