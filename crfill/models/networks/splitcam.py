...
class ReduceContextAttentionP1(nn.Module):
    ...
    def get_conv_kernel(self, x, mask=None):
        ...
        if self.norm_type == 1:
            x = x / torch.sqrt(
                (x ** 2).sum(3, keepdim=True).sum(2, keepdim=True) + 1e-8
            )
        _x = F.pad(x, (self.pd, self.pd, self.pd, self.pd), mode="replicate")
        kernel = F.unfold(
            input=_x,
            kernel_size=(self.bkg_patch_size, self.bkg_patch_size),
            stride=self.ufstride,
        )
        kernel = kernel.transpose(1, 2).view(
            batch, -1, c, self.bkg_patch_size, self.bkg_patch_size
        )
        if self.norm_type == 2:
        ...
        m = m.transpose(1, 2).view(
            batch, -1, 1, self.bkg_patch_size, self.bkg_patch_size
        )
        m = m.squeeze(2)
...
class ReduceContextAttentionP2(nn.Module):
    ...
    def get_aux_kernel(self, b):
        ...
        bkg_kernel = F.unfold(
            input=_b,
            kernel_size=(self.aux_patch_size, self.aux_patch_size),
            stride=self.ufstride_aux,
        )
        bkg_kernel = bkg_kernel.transpose(1, 2).view(
            batch, -1, c, self.aux_patch_size, self.aux_patch_size
        )
        return bkg_kernel
    ...
    def get_deconv_kernel(self, b, mask):
        ...
        msk_kernel = msk_kernel.transpose(1, 2).view(
            batch, -1, 1, self.bkg_patch_size, self.bkg_patch_size
        )
        _b = F.pad(b, (self.pd, self.pd, self.pd, self.pd), mode="replicate")
        bkg_kernel = F.unfold(
    ...
    def forward_batch(self, cos_similar, b, mask, dict_aux):
        ...
        output = batch_transposeconv2d(
            cos_similar, weight=bkg_kernel, stride=self.stride
        )

        norm_kernel = torch.ones(1, 1, self.bkg_patch_size, self.bkg_patch_size).to(
            mask.device
        )
        weight_map = torch.ones(1, 1, hs, ws).to(mask.device)
        weight_map = F.conv_transpose2d(weight_map, norm_kernel, stride=self.stride)
        mask_recon = batch_transposeconv2d(
            cos_similar, weight=msk_kernel, stride=self.stride
        )
        ...
        for k, v in dict_aux.items():
            ...
            recon = batch_transposeconv2d(
                hard_similar, weight=kernel, stride=self.stride_aux
            )
...
class AttUpLayer(nn.ModuleDict):
    def __init__(
        self, patch_size=4, grid_size=32, th=0.5, norm_type=1, is_th=False, mk=True
    ):
        super().__init__()
        self.cam_1 = ReduceContextAttentionP1(
            nn_hard=False,
            ufstride=patch_size,
            stride=patch_size,
    ...
    def unfold(self, input):
        ...
        output = F.unfold(
            input=input,
            kernel_size=(self.grid_size, self.grid_size),
            stride=self.grid_size,
        )
        n_grid = output.size(2)
        output = output.transpose(1, 2).reshape(
            b * n_grid, -1, self.grid_size, self.grid_size
        )
        return output
    ...
    def forward(self, q_im, k_im, v_im, up_im, msk_q, msk_up):
        ...
        if min(hs, ws) > self.grid_size:
            ...
            sb = (
                _similar.view(b, n_h * n_w, -1)
                .transpose(1, 2)
                .reshape(b, s_h, s_w, s_h, s_w, n_h * n_w)
            )
            ss = torch.diag_embed(sb, 0, 1, 4)
            # ss = torch.zeros(1,4,8,8,4,8,8).to(similar.device)
            # ss[0,[0,1,2,3],:,:,[0,1,2,3],:,:] = _similar
            ss = (
                ss.reshape(b, n_h, n_w, s_h, s_w, n_h * n_w, s_h, s_w)
                .transpose(2, 3)
                .reshape(b, n_h * s_h, n_w * s_w, n_h * n_w, s_h, s_w)
            )
            ss = (
                ss.reshape(b, n_h * s_h, n_w * s_w, n_h, n_w, s_h, s_w)
                .transpose(4, 5)
                .reshape(b, n_h * s_h, n_w * s_w, n_h * s_h, n_w * s_w)
            )
