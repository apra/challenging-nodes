...
class MaskCreator:
    def __init__(self, list_mask_path=None, base_mask_path=None, match_size=False):
        ...
        if list_mask_path is not None:
            ...
            msk_filenames = list(
                map(lambda x: os.path.join(base_mask_path, x.strip("\n")), filenames)
            )
    ...
    def object_shadow(self, h, w, blur_kernel=7, noise_loc=0.5, noise_range=0.05):
        ...
        mask = mask * noise
        mask = mask + (mask == 0)
        kernel = np.ones((blur_kernel, blur_kernel), np.float32) / (
    ...
    def rectangle_mask(
        self, image_height=256, image_width=256, min_hole_size=64, max_hole_size=128
    ):
    ...
    def random_mask(self, image_height=256, image_width=256, hole_range=[0, 1]):
        ...
        while True:
            ...
            def Fill(max_size):
                ...
                ww, hh = w // 2, h // 2
                x, y = np.random.randint(-ww, image_width - w + ww), np.random.randint(
                    -hh, image_height - h + hh
                )
                mask[
                    max(y, 0) : min(y + h, image_height),
                    max(x, 0) : min(x + w, image_width),
                ] = 0
            ...
            mask = np.logical_and(
                mask, 1 - self.random_brush(int(20 * coef), image_height, image_width)
            )
