...
class ArrangedoublediscModel(InpaintdoublediscModel):
    ...
    def __init__(self, opt):
        ...
        if opt.continue_train:
            self.netD_aux = util.load_network(
                self.netD_aux, "D_aux", opt.which_epoch, opt
            )
        if opt.load_base_g is not None:
            print(f"looad {opt.load_base_g}")
            self.netG.baseg = util.load_network_path(self.netG.baseg, opt.load_base_g)
        if opt.load_base_d is not None:
            print(f"looad {opt.load_base_d}")
            self.netD = util.load_network_path(self.netD, opt.load_base_d)
    ...
    def create_optimizers(self, opt):
        ...
        if opt.isTrain:
            D_params = list(self.netD.parameters()) + list(self.netD_aux.parameters())
    ...
    def compute_discriminator_loss(self, inputs, real_image, mask):
        ...
        if not self.opt.no_gan_loss:
            ...
            pred_fake, pred_real = self.discriminate(composed_aux, real_image, mask)
            D_losses["D_Fake_aux"] = self.criterionGAN(
                pred_fake, False, for_discriminator=True
            )
            D_losses["D_real_aux"] = self.criterionGAN(
                pred_real, True, for_discriminator=True
            )
            self.netD = _netD
    ...
    def compute_generator_loss(
        ...
        _netD = self.netD
        self.netD = self.netD_aux
        G_losses_aux = self.g_image_loss(
            None, aux_image, composed_image_aux, real_image, mask, None, None
        )
        self.netD = _netD
