...
class StyleGAN2Trainer:
    ...
    def run_generator_one_step(self, data, i):
        ...
        if g_regularize:
            ...
            g_regs, self.mean_path_length = self.pix2pix_model(data, mode="g_reg")
            g_reg = sum(g_regs.values()).mean()
            g_reg.backward()
            self.optimizer_G.step()
            self.g_losses = {**g_losses, **g_regs}
        bsize = inputs.size(0)
    ...
    def run_discriminator_one_step(self, data, i):
        ...
        self.d_losses = {**d_losses_real, **d_losses_fake}
        ...
        if d_regularize:
            ...
            d_reg.backward()
            self.optimizer_D.step()
            self.d_losses = {**self.d_losses, **d_regs}
