...
class Logger:
    ...
    def write_html_eval(self, base_dir):
        ...
        for i, exp in enumerate(list_dirs):
            if os.path.isdir(os.path.join(base_dir, exp)) and not exp.startswith("."):
                ...
                if k == 0:
                    _l = list(
                        filter(lambda x: not x.startswith("."), list_exp_ticks[0])
                    )
                    list_img_names += os.listdir(
                        os.path.join(base_dir, exp, "image", str(_l[0]))
                    )
                    k += 1
        output = template.render(
            list_exp_ticks=list_exp_ticks,
            list_exp_paths=list_exp_paths,
    ...
    def write_html(self):
        ...
        if len(image_tick_path) > 0:
            output = template.render(
                plotfiles=plotfiles, image_tick_path=image_tick_path, title_name=prefix
            )
        else:
