...
standard_params = f"python -u {training_script} --dataset_mode_train custom_train_vae --dataset_mode custom_train_vae " \
...
def create_range(min: Union[int, float], max: Union[int, float], num_steps: int, distr: str = 'uniform') -> Iterable:
    if distr == 'uniform':
        values = []
...
for i in range(num_tasks):
    if lines is None:
        ...
        for line in template_lines:
            lines.append(line.replace('experiment_name', f'{experiment_name}_{i//gpus_per_node}'))
